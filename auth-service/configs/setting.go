...
// AUTH_ALLOW_DEV_SECRET must be a valid bool). Load performs the remaining
...
type rawSetting struct {
	...
	AllowDevSecret bool   `env:"AUTH_ALLOW_DEV_SECRET"`
	BaseURL        string `env:"AUTH_BASE_URL" envDefault:"http://localhost:8080"`
}
...
//   - LIMEN_SECRET unset and AUTH_ALLOW_DEV_SECRET != true → error (CWE-798 guard).
...
// errors out UNLESS allowDev is true (AUTH_ALLOW_DEV_SECRET=true, local
